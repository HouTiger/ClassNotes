...
* $\alpha-\beta$搜索
假设目前我们有一个二层搜索树，每个节点都代表一张棋盘，`floor1`表示我方落子后的棋盘，`floor2`表示对方落子后的棋盘
```
floor1:		a1       a2     a3 ...
	       /  \	/   \
floor2:	     b1    b2  b3   b4 
```
先搜索`a1`的子节点`b1, b2`，得到`evaluate(b1) == 10, evaluate(b2) == 9`  
我们假定对手足够聪明，总会选择对他最有利的位置落子，可知如果我方在`a1`处落子，对方一定会在`b2`处落子，我们的最高得分为9  
再搜索`a2`的子节点`b3`，如果`evaluate(b3) == 8`，则我们就不必再搜索`b4`节点，因为如果我们在`a2`落子，对方只需在`b3`落子就使我们比之前的最高得分9要低，也就是说，这个点一定不是最优解，故而我们放弃对`a2`的子节点的搜索，转向`a3`
